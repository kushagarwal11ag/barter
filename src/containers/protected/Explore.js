...
const Explore = () => {
	...
	useEffect(() => {
		const fetchProducts = async () => {
			try {
				const res = await axios.get("/api/v1/products/all-products", {
						...
						>
							<div className="h-40 overflow-hidden">
								<Image
									...
									width={100}
									height={100}
									alt={`${product.title} of type ${product.category} by ${product.user.name}`}
							...
							<section className="p-2 border-t border-gray-300 bg-gray-100">
								<div className="flex items-center pt-2">
									<Image
										src={product.user.avatar || defaultProfile}
										width={100}
										height={100}
										alt={`Image of trader ${product.user.name}`}
